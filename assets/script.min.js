...
define('app/page/account',[], function () {
    var model = { facebook : {},
    ...
    function loadPhotos(response) {
      loadResponse(response, {
        ...
        key:"picture",
        append: function(x){
          return "<div class=\"col-sm-2\"><img class=\"center\" src=\"" + x + "\"/></div>"
        }
      });
    }
...
define('app/page/bike',[], function () {
    ...
    function finalCountdown(){
        if (count > 0){
            ...
            $("#loading").text("("+ count + ") Loading " + dots);
            count--;
        }
        else {
            var mid = "zXgGl8elY_64.k-aImo1lki6Q";
            window.location = "//www.google.com/maps/d/edit?mid=" + mid;
        }
    }

    function load () {
        window.setInterval(finalCountdown, 1000);
    };
    return load;
});

define('app/page/global',[], function () {
    var model = { facebook : {} };

    function showLoggedIn(){
        var loggedIn = true;
        $("[data-logged="+!loggedIn+"]").addClass("hidden");
        $("[data-logged="+loggedIn+"]").removeClass("hidden");
...
define('app/page/japanese',[], function () {
    var model =
    {
        hiragana :{
            "あ": "a",  "い": "i",  "う": "u", "え": "e", "お": "o",
            ...
            "た": "ta", "ち": "tsu", "つ": "tsu", "て": "te", "と": "to",
            "な": "na", "に": "ni",  "ぬ": "nu", "ね": "ne", "の": "no",
            "は": "ha", "ひ": "hi",  "ふ": "hu", "へ": "he", "ほ": "no",
            "ま": "ma", "み": "mi",  "む": "mu", "め": "me", "も": "mo",
            "や": "ya",             "ゆ": "yu",             "よ": "yo",
            "ら": "ra", "り": "ri",  "る": "ru", "れ": "re", "ろ": "ro",
            "わ": "wa", "ゐ": "wi",             "ゑ": "we", "を": "wo",
            "ん": "n",
            "が":"ga", "ぐ":"gi", "ぎ":"gu", "げ":"ge", "ご":"go",
        ...
        katakana :{
            ...
            "タ": "ta", "チ": "tsu", "ツ": "tsu", "テ": "te", "ト": "to",
            "ナ": "na", "ニ": "ni",  "ヌ": "nu", "ネ": "ne", "ノ": "no",
            "ハ": "ha", "ヒ": "hi",  "フ": "hu", "ヘ": "he", "ホ": "no",
    ...
    function load (args) {
        var lines = $(".japanese .col-sm-4");
        for(var l = 0; l < lines.length; l+=3){
            var src = $(lines[l+0]).html();
            var rom = $(lines[l+1]).html();
            var romLower = rom.toLowerCase();
            var newSrc = "";
            var newRom = "";
            var previousIndex = 0;
            var linePrefix = "l" + l + "-";
            for(var i = 0; i < src.length; i++){
                ...
                var dictionary;
                var lookup;
                var dictionries = ["hiragana", "katakana"];
                for(var d = 0; d < dictionries.length; d++){
                    dictionary = dictionries[d];
                    lookup = model[dictionary][src[i]];
                    if (lookup) {
                        break;
                    }
                }
                if (lookup){
                    var tagOpen = "<span data-id=\"" + linePrefix + i + "\" class=\"ja " + dictionary + " char-" + lookup[lookup.length-1] + "\">";
                    var tagClose = "</span>";
                    newSrc += tagOpen + src[i] + tagClose
                    var lookupIndex = romLower.indexOf(lookup, previousIndex);
                    if (lookupIndex > -1) {
                        newRom += rom.substr(previousIndex, lookupIndex - previousIndex)
                        + tagOpen
                        + rom.substr(lookupIndex, lookup.length)
                        + tagClose;
                        previousIndex = lookupIndex + lookup.length;
                    }
                }
                else{
                    newSrc += src[i];
            ...
            newRom += rom.substr(previousIndex);

            $(lines[l+0]).html(newSrc);
            $(lines[l+1]).html(newRom);
        }
        var selected = "selected";
        function getCharElements(e){
            return $("[data-id=" + $(e).data("id") + "]");
        }
        function charHoverIn(){
            getCharElements(this).addClass(selected);
        }
        function charHoverOut(){
            getCharElements(this).removeClass(selected);
        }
        $(".ja").hover(charHoverIn, charHoverOut);
    };
    return load;
});
...
define('app/page/links',["raphael"], function (raphael) {
...
define('app/page/search',[], function () {

    function fix(){
        $(".gsc-adBlock, .gsc-adBlockVertical").addClass("hidden");
        $(".gsc-thinWrapper").attr("style", "width:100%");
        $(".gsc-selected-option-container").attr("style", "width:200px !important");
        window.setTimeout(function(){
           $(".gcsc-branding").addClass("hidden");
       }, 100);
    }

    function clickFix(){

        var adBlockCheck  = window.setInterval(function(){
            var element = $(".gsc-adBlock");
            if (element && element.length){
                window.clearInterval(adBlockCheck);
                fix();
            }
        }, 100);
    }
    ...
    function load(){
        ...
        var searchLoadedCheck = window.setInterval(function(){
            var element = $(".gsc-adBlock");
            if (element && element.length){
                window.clearInterval(searchLoadedCheck);
                $(".gsc-option-menu-container").children().last().on(
                    "click",
                    clickFix);
                fix();
            }
        ...
        var cx = '003761576642892056694:wupmnsxz_3u';
...
define('app/site.app',[
    ...
    // Facebook
    (addScript(document, "script", "ad-sense-script", "//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"));
    ...
    // Facebook
    (addScript(document, "script", "facebook-jssdk", "//connect.facebook.net/en_US/sdk.js"));
    ...
    function loadDocument(){
        ...
        switch (settings.data("settings")) {
            case "bike" : bikeLoad(settings); break;
            case "japanese": japaneseLoad(settings); break;
    ...
    function loadWindow(){
        ...
        switch (settings.data("settings")) {
            ...
            case "search": searchLoad(settings); break;
        }
    }
...
require.config({
    ...
    include: ["requireLib", "jQuery", "bootstrap", "raphael"]
});
require(["app/site.app"]);

define("../scripts/app/site.bat", function(){});
